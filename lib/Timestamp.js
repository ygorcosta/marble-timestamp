...
var Timestamp = function (_Component) {
  ...
  _createClass(Timestamp, [{
    ...
    value: function attached() {
      ...
      if (this.time) {
        (0, _syntax.updateMomentSyntax)(this.type);
        var timestamp = parseInt(this.time, 10);
...
Timestamp.STATE = {
  ...
  time: _metalState.Config.number().value(0),
  title: _metalState.Config.string().internal(true).value(undefined),
  type: _metalState.Config.oneOf(['timestamp', 'duration']).value('timestamp')
};
